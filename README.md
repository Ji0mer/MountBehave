...
- `v0.2.3` 补强北天极附近 GOTO 到位判定、命令日志本地保存、平板旋转后的连接保持和星图银河背景显示。
...
| GOTO | 星图点选、目标名称搜索、RA/Dec 坐标输入，连接后发送 `:Sr...#`、`:Sd...#`、`:MS#`；极区和机械停止场景有到位兜底 |
...
7. GOTO 后 App 会自动轮询状态；控制器报告空闲时还会读取当前 RA/Dec，确认接近目标后才释放为可发起下一次 GOTO。极区目标和机械限位/停止场景会用更宽的极区阈值和静止检测兜底。也可以手动刷新或随时取消 GOTO。
...
2. 点击“开始校准”。
3. 点击“设置目标”选择当前校准星。
4. 如果 App 提示架台侧不匹配，或目标跨过中天，先点击“支架GOTO”，等待赤道仪移动后再手动居中。
5. 用方向键把目标星居中。
6. 点击“3. 居中后接受”。
7. 对剩余校准星重复以上步骤。
...
- 赤道仪模式下校准遇到架台侧不匹配时,需要先使用"支架GOTO",再手动居中并接受；OnStepX 经纬仪模式不需要架台侧处理，也不会显示极轴精调。
...
- 压缩两星/三星校准 UI，最后一颗星接受后自动保存模型；极轴精调说明与按钮关系更明确。
