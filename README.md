...
| 相机 | 拍摄星场、盲解当前指向；极轴模式下可自动拍摄、解析并转动 RA 轴完成三点测量 |
...
相机页用于拍摄星场、检测星点并盲解当前天区。勾选“极轴校准”后，点击“自动测量”会执行三点流程：拍摄并解析第 1 张，自动转动 RA 轴约 15°，拍摄并解析第 2 张，再转动 RA 轴约 15°，拍摄并解析第 3 张，随后显示极轴误差和调整方向。该流程使用 RA 轴定向移动，不使用 GOTO，以避免中天翻转或 Dec 轴参与。

自动极轴测量要求已连接赤道仪、处于赤道仪模式且未 Park。测量期间 App 会拒绝普通 GOTO、手动移动、Park/Unpark 和跟踪切换；停止、急停和取消自动测量仍保持优先。

手动曝光依赖 Android 标准 Camera2 `MANUAL_SENSOR` 能力。部分手机（例如部分华为 / EMUI 机型）虽然原生相机有专业模式，但不会向第三方 App 暴露标准手动曝光；此时 App 会禁用手动曝光并使用自动曝光拍摄，同时在日志中记录每个 camera id 的能力用于排查。
