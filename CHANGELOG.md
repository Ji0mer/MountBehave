...
- 设置页重排为设备状态优先，新增控制台式状态卡，集中显示连接、流程、跟踪、GOTO、停放和安全状态。
- 安全控制降噪：紧急停止独立突出，夜视模式改为低亮 chip，中止 GOTO / 同步状态 / 停放操作使用更清晰的层级和禁用态。
- 固件与支架、小天体星历、命令日志在设置页内降级为次级管理区域；命令日志仍保留在最底部。
- 普通操作按钮恢复克制描边，夜视按钮在日间模式下提高辨识度，同时保持暗色控制台风格。
- 星图银河背景透明度调整为 `85/255`，比初始低透明度更明显，但不压过恒星、星座线和目标标记。

## v0.2.4 - 2026-05-02

MountBehave 第六个测试版本，重点是星图同步、多点指向修正和实机连接稳定性收尾。

### 星图同步与指向修正

- 星图工具栏合并 GOTO/取消：GOTO 进行中同一个按钮变为“取消”，原取消按钮位置改为“同步”。
- 新增星图同步入口：GOTO 到位后手控居中，点击“同步”会发送 `:Q#`、`:Sr#`、`:Sd#`、`:CM#` 和跟踪恢复命令。
- 新增 `PointingCorrectionModel`：保存两星/三星模型后，星图同步会记录 `:CM#` 之后 OnStep 仍未吸收的残差，最多保留 30 个校正点，并按角距离和时间衰减对后续 GOTO 做渐进式预修正。
- 极区目标和残差小于 `0.25°` 的同步点不会写入本地模型；未保存两星/三星模型时，“同步”仍可用，但只执行 OnStep `:CM#`，不记录残差。
- 修复星图同步太快发送 `:CM#` 导致 OnStep 返回 `E8` 的问题：同步前复用 GOTO 停稳确认，等待 `:D#` 空闲且 RA/Dec 连续稳定。

### 实机稳定性

- 手控移动/停止命令增加短重试；方向停止失败时会兜底发送 `:Q#`，减少松手或停止时因瞬时 WiFi 抖动导致的假断开。
- 改进连接丢失判定，避免把普通读取超时误判为硬断网，同时仍能识别 connect timeout、ECONNRESET、EPIPE 等真实网络故障。
- 校准残差超过 `1°` 时只提示风险，不再强制取消或阻止保存模型，便于现场继续测试。

### 文档与发布

- README、测试人员教学和 plan 更新为星图同步流程，并说明本项目由 Codex 与 Claude Code 辅助完成。
...
- 连续两次 GOTO 停止但离目标仍超过 `2°`，且 OnStep 报告 `GE=20/21` 或 `E6/E7` 时，App 会暂停新的 GOTO 并提示先用手控移出限位/危险位置；`GE=15` / `E1` 等低空或地平线下错误不会触发暂停。
- GOTO 发送前先发 `:Q#`，并等待连续两次空闲且 RA/Dec 稳定后再继续，避免上一段停止/减速未落稳；这会让正常 GOTO 前置等待略长，但能减少急停或限位后的假到达。`:Sr#`、`:Sd#`、`:MS#` 改用短回复读取，正确处理 `:MS#` 的 `0-9` 单字符返回，减少 socket 超时和连续 GOTO 卡顿。
- 手动 GOTO、校准 GOTO、星图同步和其他会移动赤道仪的操作会自动把星图时间恢复为现在，避免用历史/未来时刻的动态天体坐标控制赤道仪。
...
- 星图工具栏合并 GOTO/取消：GOTO 进行中同一个按钮变为“取消”；原取消按钮位置改为“同步”。
- 新增星图同步入口：用户 GOTO 后手动居中并点击“同步”会发送 `:CM#`；保存两星/三星模型后，App 会按同步后的残差维护最多 30 个局部校正点，并在后续 GOTO 前做渐进式指向修正。
...
- 仍保留“选择位置保存”和“分享”；分享时会把当天文件日志、崩溃日志和最近内存日志合并为单个 `.txt` 附件，并显式授予接收 App 读取权限。

### 校准与 UI

- 两星/三星校准状态移动到顶部状态文本框，校准操作压缩为更少行数。
- 默认校准模式改为“两星校准”，更贴近日常 GOTO 建模入口。
- 移除独立“支架GOTO”按钮：第一颗校准星若需要赤道仪转到另一侧，会在“居中后接受”时弹出确认并自动发起必要的 GOTO。
- 第一颗星需要换侧时，支持 OnStepX preferred pier side 的固件会临时设置 `:SX96#` 引导真实翻转，并在 GOTO 后恢复原值；不支持的固件自动降级并写入诊断日志。
- 校准接受后会记录目标残差；若任一颗星接受后仍偏离超过 `1°`，App 会提示可能未居中，但仍允许用户继续校准并保存模型。
- 校准启动失败、主动结束、busy 状态取消和 WiFi 硬断线路径补强；短暂断线后会尽量保留可恢复的未完成校准会话。
