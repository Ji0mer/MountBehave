...
- 三星后极轴精调、三星校准、OnStepX 架台切换、Home 原生命令仍需要更多实机验证。
...
- 删除或重写:
  ...
  - `currentHomeRaHours`
  - `updateHomeViews`
- `Set home` 使用可选短回包发送 `:hC#`:若固件回 `1` 视为成功,回 `0`/`E...` 视为拒绝;若实机静默关闭/无回包,以命令 flush 成功视为已请求成功。
- `Return home` 使用可选短回包发送 `:hF#`:若固件回 `1` 视为成功,回 `0`/`E...` 视为拒绝;若实机静默关闭/无回包,以命令 flush 成功视为已请求成功并进入 Home 查找/GOTO 状态。
...
- Mock 对 `:hC#` / `:hF#` 回 `1` 或静默无回包,App 显示成功。
- Mock 回 `0`,App 显示失败。
