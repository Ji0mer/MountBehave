...
| 小行星 / 彗星 (扩展) | 用户按需下载 | App 内"同步亮小行星 / 添加彗星"按钮 → 同 JPL SBDB |
| 银河背景 | 去除亮星的银河全天图 | NASA SVS 4851 `milkyway_2020_4k_print.jpg`,内置为 `app/src/main/res/drawable-nodpi/milkyway.jpg` |
...
- 星图工具栏"图层"按钮控制可切换天体的可见性。**恒星和银河背景始终显示**(避免用户误关后无法定位);可切换 7 项:**星座连线 / 太阳系天体**(默认 ON)、**星团 / 星云 / 星系**(三个独立 DSO 类别)、**小行星 / 彗星**(两个独立类别)默认 OFF。设置在当前会话内有效,关闭再打开会重置回默认。
- 星图工具栏"时间"按钮可设置星图日期/时间或恢复为"现在"。GOTO、同步、校准等会移动赤道仪的操作会自动把星图时间恢复为当前时刻,避免历史/未来时刻的太阳系目标被直接用于控制赤道仪。
- App 顶层菜单包含**设置**、**连接/同步**、**手控**、**星图**。设置页顶部显示设备状态卡，并收纳安全控制、固件/支架模式、小天体星历和命令日志；连接/同步页收纳 WiFi 连接、观测地与时间、跟踪。
- 广视场下只显示较亮恒星，避免星图过密。
...
- 银河背景只用于目视寻星时的方位参考。贴图按 SVS 页面说明使用天赤道坐标系,中心 RA 0h / Dec 0°,RA 向左增加,在星图中以 `85/255` 透明度叠加。
...
数据更新策略:VSOP87D 表 2005 年发布,数十年不需更新。小天体基线随 App 发布;新发现彗星和当年观测的暗弱小行星请用设置页"同步亮小行星 / 添加彗星"在线刷新。当前 UI 不再提供视星等上限滑块,以用户主动输入或下载动作为准。
...
- NASA SVS 银河背景按 NASA Images and Media Usage Guidelines 使用。NASA 内容通常不受美国版权限制,可用于教育/信息用途；需要注明 NASA/Goddard Space Flight Center Scientific Visualization Studio,不能暗示 NASA 背书,NASA 标识/Logo 另受限制。若页面单独标记第三方版权,以该标记为准。
